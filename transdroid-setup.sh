...
mkdir $tmpdir
wget -qO ~/$tmpdir/td-settings.json https://raw.githubusercontent.com/frankthetank7254/feral/master/settings.json
 
echo  "Please enter the ruTorrent password from your Account overview page:"
read rtpass
 
sed "s/USERNAME-CHANGEME/$(whoami)/" ~/$tmpdir/td-settings.json | sed "s/HOSTNAME-CHANGEME/$(hostname -f)/" | sed "s/RTORRENTPASSWORD-CHANGEME/$rtpass/" | sed "s/SERVERNAME-CHANGEME/$(hostname)-rutorrent/" $
